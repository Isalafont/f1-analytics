...
namespace :f1 do
  namespace :news do
    ...
    task :add_team, %i[team_name category title] => :environment do |_t, args|
    ...
    task :add_driver, %i[driver_code category title] => :environment do |_t, args|
  ...
  namespace :weather do
    ...
    task :add, %i[round session] => :environment do |_t, args|
    ...
    task :show, [:round] => :environment do |_t, args|
      season = ENV["SEASON"]&.to_i || Date.current.year
      race = Race.find_by(season: season, round: args[:round].to_i)
